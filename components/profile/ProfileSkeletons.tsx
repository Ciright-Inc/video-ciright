        ...
        <ul className="divide-y divide-hairline-soft">
          {Array.from({ length: 5 }).map((_, i) => (
            <li key={i} className="flex items-center gap-3 p-3">
              <Skeleton className="h-12 w-20 shrink-0 rounded-[var(--radius-sm)]" />
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        {Array.from({ length: 2 }).map((_, i) => (
          <ProfileSurfaceCard key={i}>
            ...
            <Skeleton className="mt-4 h-64 w-full rounded-[var(--radius-md)]" />
          </ProfileSurfaceCard>
        ))}
      </div>
    </div>
            ...
            <div className="flex flex-col gap-1">
              {Array.from({ length: section === 0 ? 4 : 3 }).map((_, i) => (
                <div key={i} className="flex gap-2 py-1">
                  <Skeleton className="aspect-video w-[168px] shrink-0 rounded-[var(--radius-md)]" />
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        ...
        <Skeleton className="h-10 w-full rounded-[var(--radius-md)] md:w-40" />
      </div>
